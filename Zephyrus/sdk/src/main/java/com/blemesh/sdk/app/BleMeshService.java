...
import com.blemesh.sdk.session.GraphMessage;
import com.blemesh.sdk.session.SessionManager;
...
//        mPeersGraph = null;
    ...
    public class ServiceBinder extends Binder {
        ...
        public void send(byte[] data, Peer recipient) {
            ...
            addOutgoingTransfer(new OutgoingTransfer(data, recipient, next_reply_node, localPeer.getAlias(), sessionManager));
        }
         ...
         *
         * @return either {@link com.blemesh.sdk.transport.ble.BLETransport#TRANSPORT_CODE},
    ...
    private Peer getNextReply(Peer recipient){
        ...
        next_reply_node = mPeersGraph.getNextReply(recipient.getAlias());
        if(next_reply_node.equals(recipient))
            Timber.d("Next hop is destination recipient %s : %s",recipient.getAlias(),recipient.getAlias());
        else
            Timber.d("Desc is %s : %s, next replay is %s : %s",
                    recipient.getAlias(), recipient.getAlias(),
                    next_reply_node.getAlias(), next_reply_node.getAlias());
...
//            }
    ...
    public void directPeerStatusUpdated(@NonNull final Peer peer, @NonNull final Transport.ConnectionStatus newStatus, final boolean isHost) {
        ...
        switch (newStatus) {
            case CONNECTED:
                Timber.d("New direct remote device alias:%s", peer.getAlias());
...
//                }
            ...
            case DISCONNECTED:
                Timber.d("Device has disconnected, alias::%s ", peer.getAlias());
...
//                        remoteGraph.displayGraph();
                        ...
                        remoteGraph.getEdgeMatrix().get(sender.getAlias()).put(
                                localPeer.getAlias(),
                                mPeersGraph.getEdgeMatrix().get(sender.getAlias()).get(localPeer.getAlias())
                        );
...
//                }
            ...
            case DataTransferMessage.HEADER_TYPE:
                ...
                if(LocalPeer.getLocalAlias().equals(dataTransferMessage.getDesc_alias())){
                    // Reach the desc node
                    ...
                    forward(message,mPeersGraph.getVertexList().get(dataTransferMessage.getDesc_alias()));
                }
                break;
        }
    }
        ...
        foregroundHandler.post(new Runnable() {
            ...
            public void run() {
                if (callback != null) {
                    ...
                    vertexes.remove(localPeer.getAlias());
    ...
    public void messageSentToPeer(@NonNull SessionMessage message, @NonNull final Peer recipient, Exception exception) {
        ...
        if (message.getType().equals(DataTransferMessage.HEADER_TYPE)) {
            ...
            // No action is required for DataTransferMessage. Report complete
            final Peer desc = mPeersGraph.getVertexList().get(((DataTransferMessage)message).getDesc_alias());
