...
public class LocalGraph extends PeersGraph {
    ...
    public LocalGraph(Peer LocalNode) {
        ...
        addMatrixRow(LocalNode.getAlias());
//        insertEdge(new PeersEdge(LocalNode.getAddress(),LocalNode.getAddress(),0));
    ...
    synchronized public void newDirectRemote(Peer remoteNode) {
        Timber.d("New direct remote peer: %s", remoteNode.getAlias());
        if (!hasVertex(remoteNode)) {
            insertVertex(remoteNode);
        }
        if (!hasMatrixRow(remoteNode.getAlias())) {
            addMatrixRow(remoteNode.getAlias());
        }
        insertEdge(new PeersEdge(remoteNode.getAlias(),
                localNode.getAlias(),
                remoteNode.getRssi()));
        unMergedNewNodes.add(remoteNode.getAlias());
    }

    synchronized public void lostDirectRemote(Peer remoteNode){
        Timber.d("Lost direct remote peer: %s", remoteNode.getAlias());
        deleteEdge(localNode.getAlias(),remoteNode.getAlias());
        if(unMergedNewNodes.contains(remoteNode.getAlias()))
            unMergedNewNodes.remove(remoteNode.getAlias());
        else
    ...
    synchronized public void mergeGarph(Peer remoteNode, PeersGraph otherGraph) {
        Timber.d("remote node is: %s", remoteNode);
        for(Peer node : otherGraph.getVertexList().values()){
            if (!hasVertex(node)) {
                insertVertex(node);
                addMatrixRow(node.getAlias());
            }
        }
    ...
    public void calCluateShortestPath(){
        ...
        //initialization
        Shortest_Path_Info_Map.get(localNode.getAlias()).setHops(0).setRssi(0);
        String this_visit, next_visit;
        next_visit = localNode.getAlias();
...
//        }
    ...
    public Peer getNextReply(String alias){
        if(LocalPeer.getLocalAlias().equals(alias)){
            return null;
        }
        ...
        Timber.d("getNextReply desc is %s, next replay is %s",
                alias,
                next_reply.getAlias());
        return next_reply;
    }
